...
import torch
from torch_geometric.datasets import PPI, Amazon, Planetoid, WebKB, Coauthor
...
def dataloader(config):
    ...
    if config.data.data in ['cs', 'physics']:
        path = osp.join(osp.dirname("D:\\"), 'data',  config.data.data)
        dataset = Coauthor(path, config.data.data) 
        data = dataset[0]
        nlabel = config.data.nlabel
        y = F.one_hot(data.y, nlabel).float()
        idx = torch.randperm(data.x.size()[0])
        train_idx = idx[:idx.size()[0]//10]
        val_idx = idx[idx.size()[0]//10:idx.size()[0]//10*2]
        test_idx = idx[idx.size()[0]//10*2:]
